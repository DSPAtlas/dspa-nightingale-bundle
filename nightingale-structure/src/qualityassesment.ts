...
import { mmCIF_Schema } from './mmCIFschema';
...
namespace QualityAssessment {
    ...
    export function isApplicable(model?: Model, localMetricName?: 'lipScore' |'pLDDT' | 'qmean'): boolean {
    ...
    export const symbols = {
        ...
        lipScore: QuerySymbolRuntime.Dynamic(CustomPropSymbol('ma', 'quality-assessment.lipScore', Type.Num),
        ctx => {
            const { unit, element } = ctx.element;
            if (!Unit.isAtomic(unit)) return -1;
            const qualityAssessment = QualityAssessmentProvider.get(unit.model).value;
