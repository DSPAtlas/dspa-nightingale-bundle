...
class NightingaleSequenceHeatmap extends withManager(
  ...
  render() {
    ...
    if (this.heatmapData) {
      ...
      return html` <style>
        ...
        <div id="container" style="width: ${this.width}px; max-width: ${this.width}px;">
  ...
  bindHeatmapEvents() {
    ...
    this.heatmapInstance.events.hover.subscribe((d) => {
      let toSend = null;
      if (d && d.cell && d.cell.x !== undefined) {
        toSend = `${d.cell.x}:${d.cell.x}`;
      }
      {
        const hm = this.heatmapInstance as any;
        const wtc = hm?.state?.scales?.worldToCanvas?.x;
        const highlightPos = d?.cell?.x;
        const heatmapPixelX = highlightPos !== undefined && wtc ? wtc(highlightPos - 1) : null;
        // Compute what the sequence track's xScale would give for the same position
        const W = this.getWidthWithMargins();
        const ds = this["display-start"] || 1;
        const de = this["display-end"] || this.length || 1;
        // sequence originXScale: [1, length+1] -> [0, W]
        // after zoom: [ds, de+1] -> [0, W]  (approximately)
        const seqPixelX = highlightPos !== undefined ? (highlightPos - ds) / (de + 1 - ds) * W : null;
        console.log("[SEQ-HEATMAP HOVER]", {
          pos: highlightPos, toSend,
          heatmapPx: heatmapPixelX?.toFixed(2),
          seqPx: seqPixelX?.toFixed(2),
          diff: heatmapPixelX != null && seqPixelX != null ? (heatmapPixelX - seqPixelX).toFixed(2) : null,
          W, ds, de,
        });
      }
      this.dispatchEvent(
  ...
  triggerHeatmapHighlight() {
    ...
    // Use a dedicated class name to avoid collision with the heatmap's own
    // MarkerY (row highlight) which shares the same SVG and d3 selection.
    const className = "nightingale-highlight-marker";
        ...
        (
          ...
          enter
            ...
            .attr("x", (d: SegmentType) => {
              // convert 1-based Nightingale highlight position to 0-based heatmap world index
